...
def fetch_github_stats(username="sogmax"):
...
def update_readme(selected_image, colors):
    ...
    try:
        ...
        last_login = datetime.utcnow().strftime("%a %b %d %H:%M:%S UTC %Y")
        content = content.replace("{{ LAST_LOGIN }}", last_login)
    ...
    for color in colors:
        ...
        color_imgs.append(f'<img alt="{color}" src="https://raw.githubusercontent.com/sogmax/sogmax/main/img/{hex_clean}.png" width="25" height="20" />')
